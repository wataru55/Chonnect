...
class BLECentralManager: NSObject, ObservableObject, CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if let receivedDocumentId = advertisementData[CBAdvertisementDataLocalNameKey] as? String {
            ...
            DispatchQueue.main.async {
                // 受信したuserIdをRealmに保存
                // リアルタイムデータ
                RealtimeDataManager.shared.storeRealtimeData(receivedUserId: receivedDocumentId, date: Date(), rssi: RSSI.intValue)
                // 履歴データ
                HistoryManager.shared.storeHistoryData(receivedDocumentId, date: Date())
            }
        }
    }
    ...
    func startCleanupTimer() {
        cleanupTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { _ in
            Task { @MainActor in
                RealtimeDataManager.shared.removeOutdatedData()
            }
        }
    }
}
