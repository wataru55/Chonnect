...
actor UserProfileRepository {
    ...
    func saveOrUpdate(profileData: ProfileData) async {
        do {
            try await realm.asyncWrite {
                ...
                profileCache.follows.append(objectsIn: profileData.follows.map { UserCache(from: $0) })
                profileCache.followers.append(objectsIn: profileData.followers.map { UserCache(from: $0) })
                profileCache.skillTags.append(objectsIn: profileData.skillTags.map { WordElementCache(from: $0) })
    ...
    func fetch(userId: String) -> CacheResult {
        guard let profileCache = realm.object(ofType: UserProfileDataCache.self, forPrimaryKey: userId) else {
        ...
        let data = ProfileData(user: user, follows: follows, followers: followers, skillTags: skillTags, ogp: ogp)
