...
class AuthService {
    ...
    init() {
        ...
        Task {
            do {
                self.userRepository = try await UserRepository()
                self.userSession = Auth.auth().currentUser
                try await initializeCurrentUser()
            } catch {
                print("❌ [AuthService] Failed to initialize dependencies: \(error)")
            }
        }
    }

    /// currentUserプロパティの変更を監視して自動的にキャッシュ保存
    private func setupCurrentUserObserver() {
        print("🚀 [CACHE] Setting up currentUser observer for automatic cache saving")
        $currentUser
            .compactMap { $0 }  // nilでない値のみ通過
            .removeDuplicates()  // 同じユーザーの重複を除去
            .sink { [weak self] user in
                print(
                    "👀 [CACHE] currentUser changed detected, triggering cache save for: \(user.username)"
                )
                Task {
                    await self?.saveCurrentUserToCache(user)
                }
            }
            .store(in: &cancellables)
        print("✅ [CACHE] currentUser observer setup completed")
    }
    ...
    func initializeCurrentUser() async throws {
        ...
        // オブザーバーが自動的にキャッシュ保存を実行
        case .failure(let error):
            throw error
        }
    }

    /// CurrentUserをキャッシュに保存する
    private func saveCurrentUserToCache(_ user: User) async {
        guard let userRepository = self.userRepository else {
            print("❌ [CACHE] UserRepository not initialized. Cannot save cache.")
            return
        }

        await userRepository.saveOrUpdate(user: user)
        print("✅ [CACHE] Successfully saved current user to cache: \(user.username)")
    }
    ...
    func login(email: String, password: String) async -> Result<Bool, AuthError> {
        do {
            ...
            print(
                "DEBUG: ログイン失敗 - domain: \(error.domain), code: \(error.code), message: \(error.localizedDescription)"
            )
    ...
    private func checkEmailVerification(for user: FirebaseAuth.User) async -> Result<
        Bool, AuthError
    > {
        do {
    ...
    func reAuthenticate(email: String, password: String) async -> Result<Bool, AuthError> {
        ...
        do {
            ...
            print(
                "DEBUG: 再認証失敗 - domain: \(error.domain), code: \(error.code), message: \(error.localizedDescription)"
            )
            switch error.code {
            case AuthErrorCode.invalidEmail.rawValue:
                return .failure(.invalidEmail)
    ...
    // 新規ユーザを作成する関数
    func createUser(email: String, password: String, username: String) async -> Result<
        Bool, AuthError
    > {
        do {
    ...
    func initAddToFireStore(username: String) async -> Result<Bool, AuthError> {
        ...
        do {
            ...
            let result = await createUserDocument(
                id: documentId, uid: currentUser.uid, username: username, isPrivate: true)
            switch result {
            case .success:
    ...
    private func createUserDocument(id: String, uid: String, username: String, isPrivate: Bool)
        async -> Result<Bool, AuthError>
    {
        let user = User(
            id: id, uid: uid, username: username, isPrivate: isPrivate, snsLinks: [:],
            attributes: [], interestTags: [])
        self.currentUser = user
        // オブザーバーが自動的にキャッシュ保存を実行

        do {
            let encodedUser = try Firestore.Encoder().encode(user)
            try await Firestore.firestore().collection("users").document(user.id).setData(
                encodedUser)
            return .success(true)
        } catch let error as NSError {
