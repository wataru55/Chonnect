    ...
    var body: some View {
        NavigationStack(path: $path) {
            ZStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    ZStack(alignment: .top) {
                        BackgroundImageView(
                            imageUrl: viewModel.user.backgroundImageUrl, height: 500,
                            isGradient: true)
                        ...
                        VStack(spacing: 0) {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                Attributes(
                                    attributes: viewModel.user.attributes,
                                    option: AttributeOption.profile)
            ...
            .navigationDestination(for: UserDatePair.self) { pairData in
                ProfileView(
                    user: pairData.user, currentUser: viewModel.user, date: pairData.date,
                    isShowFollowButton: true, isShowDateButton: true)
            }
            .navigationDestination(for: User.self) { user in
                ProfileView(
                    user: user, currentUser: viewModel.user, date: nil,
